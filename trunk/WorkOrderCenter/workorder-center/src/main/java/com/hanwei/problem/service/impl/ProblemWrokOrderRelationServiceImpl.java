 ...
 */
@Service
public class ProblemWrokOrderRelationServiceImpl extends ServiceImpl<ProblemWrokOrderRelationMapper, ProblemWrokOrderRelation> implements IProblemWrokOrderRelationService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, ProblemWrokOrderRelation problemWrokOrderRelation) {
        QueryWrapper<ProblemWrokOrderRelation> queryWrapper = QueryGenerator.initQueryWrapper(problemWrokOrderRelation, paramMap);
        List<ProblemWrokOrderRelation> list = list(queryWrapper);
        EasyExcel.write(outputStream, ProblemWrokOrderRelation.class).sheet("问题工单关联信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, ProblemWrokOrderRelation problemWrokOrderRelation) {
        QueryWrapper<ProblemWrokOrderRelation> queryWrapper = QueryGenerator.initQueryWrapper(problemWrokOrderRelation, paramMap);
        List<ProblemWrokOrderRelation> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<ProblemWrokOrderRelation> result = listJson.stream().map(e -> JSON.toJavaObject(e,ProblemWrokOrderRelation.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), ProblemWrokOrderRelation.class, new ReadListener<ProblemWrokOrderRelation>() {

                private List<ProblemWrokOrderRelation> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(ProblemWrokOrderRelation data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/ProblemWrokOrderRelation.xlsx");
            // 方式一：路径
