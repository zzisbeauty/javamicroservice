package com.hanwei.attachment.service.impl;
...
public class AttachmentServiceImpl extends ServiceImpl<AttachmentMapper, Attachment> implements IAttachmentService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Attachment attachment) {
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Attachment attachment) {
        QueryWrapper<Attachment> queryWrapper = QueryGenerator.initQueryWrapper(attachment, paramMap);
        List<Attachment> list = list(queryWrapper);
        //字典值转换
...
//        List<Attachment> result = listJson.stream().map(e -> JSON.toJavaObject(e,Attachment.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Attachment.class, new ReadListener<Attachment>() {

                private List<Attachment> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Attachment data, AnalysisContext context) {
                    //---------------处理字典转义
