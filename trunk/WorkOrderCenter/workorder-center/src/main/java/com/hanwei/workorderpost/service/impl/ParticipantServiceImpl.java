...
import com.hanwei.workorderpost.mapper.ParticipantMapper;
...
public class ParticipantServiceImpl extends ServiceImpl<ParticipantMapper, Participant> implements IParticipantService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Participant participant) {
        QueryWrapper<Participant> queryWrapper = QueryGenerator.initQueryWrapper(participant, paramMap);
        List<Participant> list = list(queryWrapper);
        EasyExcel.write(outputStream, Participant.class).sheet("工单后处理记录-参与人信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Participant participant) {
        QueryWrapper<Participant> queryWrapper = QueryGenerator.initQueryWrapper(participant, paramMap);
        List<Participant> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Participant> result = listJson.stream().map(e -> JSON.toJavaObject(e,Participant.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Participant.class, new ReadListener<Participant>() {

                private List<Participant> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Participant data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Participant.xlsx");
            // 方式一：路径
