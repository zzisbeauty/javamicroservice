...
import { handleEdit, handleAdd } from '../UrgeInfoApi'
...
const dialogSchemas = reactive<FormSchema[]>([
   ...
   {
      label: '操作节点ID（可选，标识流程中哪个节点被催办）',
      field: 'operationNodeId',
      component: 'Input',
      colProps: {
        span: 24
      }
...
const rules = reactive({
        ...
        operationNodeId: [required()],
        urgeAt: [required()],
        urgeContent: [required()],
        assigneeCode: [required()],
        assigneeName: [required()],
        isRead: [required()],
})
