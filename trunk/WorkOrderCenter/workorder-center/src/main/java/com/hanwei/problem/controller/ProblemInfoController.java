...
public class ProblemInfoController extends BaseController<ProblemInfo, IProblemInfoService> {
	 ...
	 * @return
	 */
	@AutoLog(value = "问题信息管理表-分页列表查询")
	@Operation(summary="问题信息管理表-分页列表查询")
	@RequestMapping(value = "/list", method = {RequestMethod.POST,RequestMethod.GET})
	public Result<?> queryPageList(ProblemInfo problemInfo,
								   @RequestParam(name="pageNo", defaultValue="1") Integer pageNo,
								   @RequestParam(name="pageSize", defaultValue="10") Integer pageSize,
								   HttpServletRequest req) {
		QueryWrapper<ProblemInfo> queryWrapper = QueryGenerator.initQueryWrapper(problemInfo, req.getParameterMap());
	...
	@Operation(summary="问题信息管理表-添加")
	@PostMapping(value = "/add")
	public Result<?> add(@RequestBody ProblemInfo problemInfo) {
		Boolean flag = problemInfoService.save(problemInfo);
		if(flag){
	...
	public Result<?> edit(@RequestBody ProblemInfo problemInfo) {
		Boolean flag = problemInfoService.updateById(problemInfo);
		if(flag){
	...
	public Result<?> delete(@RequestParam(name="id",required=true) String id) {
		Boolean flag = problemInfoService.removeById(id);
		if(flag){
	...
	public Result<?> deleteBatch(@RequestParam(name="ids",required=true) String ids) {
		Boolean flag = this.problemInfoService.removeByIds(Arrays.asList(ids.split(",")));
		if(flag){
	 ...
	 * @return
	 */
	@AutoLog(value = "问题信息管理表-通过id查询")
	@Operation(summary="问题信息管理表-通过id查询")
	@GetMapping(value = "/queryById")
	public Result<?> queryById(@RequestParam(name="id",required=true) String id) {
		ProblemInfo problemInfo = problemInfoService.getById(id);
