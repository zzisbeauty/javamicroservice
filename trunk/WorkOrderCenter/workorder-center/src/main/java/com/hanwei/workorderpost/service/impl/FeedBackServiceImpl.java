...
import com.hanwei.workorderpost.mapper.FeedBackMapper;
...
public class FeedBackServiceImpl extends ServiceImpl<FeedBackMapper, FeedBack> implements IFeedBackService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, FeedBack feedBack) {
        QueryWrapper<FeedBack> queryWrapper = QueryGenerator.initQueryWrapper(feedBack, paramMap);
        List<FeedBack> list = list(queryWrapper);
        EasyExcel.write(outputStream, FeedBack.class).sheet("工单后处理记录-回访信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, FeedBack feedBack) {
        QueryWrapper<FeedBack> queryWrapper = QueryGenerator.initQueryWrapper(feedBack, paramMap);
        List<FeedBack> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<FeedBack> result = listJson.stream().map(e -> JSON.toJavaObject(e,FeedBack.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), FeedBack.class, new ReadListener<FeedBack>() {

                private List<FeedBack> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(FeedBack data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/FeedBack.xlsx");
            // 方式一：路径
