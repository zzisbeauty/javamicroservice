package com.hanwei.workorderpost.service.impl;
...
import com.hanwei.workorderpost.mapper.UrgeInfoMapper;
 ...
 */
@Service
public class UrgeInfoServiceImpl extends ServiceImpl<UrgeInfoMapper, UrgeInfo> implements IUrgeInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, UrgeInfo urgeInfo) {
        QueryWrapper<UrgeInfo> queryWrapper = QueryGenerator.initQueryWrapper(urgeInfo, paramMap);
        List<UrgeInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, UrgeInfo.class).sheet("工单后处理记录-催办信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, UrgeInfo urgeInfo) {
        QueryWrapper<UrgeInfo> queryWrapper = QueryGenerator.initQueryWrapper(urgeInfo, paramMap);
        List<UrgeInfo> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<UrgeInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,UrgeInfo.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), UrgeInfo.class, new ReadListener<UrgeInfo>() {

                private List<UrgeInfo> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(UrgeInfo data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/UrgeInfo.xlsx");
            // 方式一：路径
