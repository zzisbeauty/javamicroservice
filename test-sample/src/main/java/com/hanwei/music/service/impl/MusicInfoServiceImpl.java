package com.hanwei.music.service.impl;
...
import com.hanwei.music.mapper.MusicInfoMapper;
...
public class MusicInfoServiceImpl extends ServiceImpl<MusicInfoMapper, MusicInfo> implements IMusicInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, MusicInfo musicInfo) {
        QueryWrapper<MusicInfo> queryWrapper = QueryGenerator.initQueryWrapper(musicInfo, paramMap);
        List<MusicInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, MusicInfo.class).sheet("音乐专辑管理器").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, MusicInfo musicInfo) {
        QueryWrapper<MusicInfo> queryWrapper = QueryGenerator.initQueryWrapper(musicInfo, paramMap);
        List<MusicInfo> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<MusicInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,MusicInfo.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), MusicInfo.class, new ReadListener<MusicInfo>() {

                private List<MusicInfo> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(MusicInfo data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/MusicInfo.xlsx");
            // 方式一：路径
