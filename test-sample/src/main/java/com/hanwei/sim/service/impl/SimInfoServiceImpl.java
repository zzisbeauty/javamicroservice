package com.hanwei.sim.service.impl;
...
import com.hanwei.sim.mapper.SimInfoMapper;
 ...
 */
@Service
public class SimInfoServiceImpl extends ServiceImpl<SimInfoMapper, SimInfo> implements ISimInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, SimInfo simInfo) {
        QueryWrapper<SimInfo> queryWrapper = QueryGenerator.initQueryWrapper(simInfo, paramMap);
        List<SimInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, SimInfo.class).sheet("SIM卡管理器").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, SimInfo simInfo) {
        QueryWrapper<SimInfo> queryWrapper = QueryGenerator.initQueryWrapper(simInfo, paramMap);
        List<SimInfo> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<SimInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,SimInfo.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), SimInfo.class, new ReadListener<SimInfo>() {

                private List<SimInfo> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(SimInfo data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/SimInfo.xlsx");
            // 方式一：路径
