...
import com.hanwei.core.filter.FileTypeFilter;
...
public class CommonUtils {
    ...
    public static String getBaseUrl(HttpServletRequest request) {
        ...
        String xGatewayBasePath = request.getHeader(CommonConstant.X_GATEWAY_BASE_PATH);
    ...
    public static void copyProperties(Object source, Object target){
        ...
        for (Field sourceField : sourceClass.getDeclaredFields()) {
            sourceField.setAccessible(true); // 设置属性为可访问

            // 尝试获取目标对象的相同名称属性
            try {
                Field targetField = targetClass.getDeclaredField(sourceField.getName());
                // 如果为空则跳过
                if(!Optional.ofNullable(targetField).isPresent()){
                    continue;
                }
                targetField.setAccessible(true); // 设置属性为可访问
                ...
                log.error("拷贝失败 属性不可访问",e.getMessage());
                e.printStackTrace();
            }
        }
    }
