...
import com.hanwei.statistics.mapper.AnnualMapper;
...
public class AnnualServiceImpl extends ServiceImpl<AnnualMapper, Annual> implements IAnnualService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Annual annual) {
        QueryWrapper<Annual> queryWrapper = QueryGenerator.initQueryWrapper(annual, paramMap);
        List<Annual> list = list(queryWrapper);
        EasyExcel.write(outputStream, Annual.class).sheet("年度汇聚信息表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Annual annual) {
        QueryWrapper<Annual> queryWrapper = QueryGenerator.initQueryWrapper(annual, paramMap);
        List<Annual> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Annual> result = listJson.stream().map(e -> JSON.toJavaObject(e,Annual.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Annual.class, new ReadListener<Annual>() {

                private List<Annual> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Annual data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Annual.xlsx");
            // 方式一：路径
