...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  },
  {
   ...
   {
      label: '汇聚时间（如 2025-01-01 00:00:00 表示 1月起始，精确到秒）',
...
const getFile = async () => {
  ...
  try {
    ...
    handleGetFile().then((res: any) => {
      ...
      fileExport(fileData, '月度汇聚信息表.xlsx')
    })
  } catch (error) {
...
const doExport = () => {
  handleExport().then((res: any) => {
    if (res.success) {
      fileExport(res.result, '月度汇聚信息表.xlsx')
      useMessage('success', res.message)
    } else {
