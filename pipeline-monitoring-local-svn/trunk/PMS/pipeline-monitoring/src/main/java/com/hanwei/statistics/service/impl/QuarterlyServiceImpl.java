...
import com.hanwei.statistics.mapper.QuarterlyMapper;
 ...
 */
@Service
public class QuarterlyServiceImpl extends ServiceImpl<QuarterlyMapper, Quarterly> implements IQuarterlyService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Quarterly quarterly) {
        QueryWrapper<Quarterly> queryWrapper = QueryGenerator.initQueryWrapper(quarterly, paramMap);
        List<Quarterly> list = list(queryWrapper);
        EasyExcel.write(outputStream, Quarterly.class).sheet("季度汇聚信息表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Quarterly quarterly) {
        QueryWrapper<Quarterly> queryWrapper = QueryGenerator.initQueryWrapper(quarterly, paramMap);
        List<Quarterly> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Quarterly> result = listJson.stream().map(e -> JSON.toJavaObject(e,Quarterly.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Quarterly.class, new ReadListener<Quarterly>() {

                private List<Quarterly> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Quarterly data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Quarterly.xlsx");
            // 方式一：路径
