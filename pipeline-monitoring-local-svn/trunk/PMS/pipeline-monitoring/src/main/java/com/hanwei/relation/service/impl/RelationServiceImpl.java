package com.hanwei.relation.service.impl;
...
import com.hanwei.relation.mapper.RelationMapper;
...
public class RelationServiceImpl extends ServiceImpl<RelationMapper, Relation> implements IRelationService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Relation relation) {
        QueryWrapper<Relation> queryWrapper = QueryGenerator.initQueryWrapper(relation, paramMap);
        List<Relation> list = list(queryWrapper);
        EasyExcel.write(outputStream, Relation.class).sheet("关联信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Relation relation) {
        QueryWrapper<Relation> queryWrapper = QueryGenerator.initQueryWrapper(relation, paramMap);
        List<Relation> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Relation> result = listJson.stream().map(e -> JSON.toJavaObject(e,Relation.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Relation.class, new ReadListener<Relation>() {

                private List<Relation> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Relation data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Relation.xlsx");
            // 方式一：路径
