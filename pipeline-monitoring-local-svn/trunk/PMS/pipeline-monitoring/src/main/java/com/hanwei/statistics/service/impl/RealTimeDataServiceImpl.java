...
import com.hanwei.statistics.mapper.RealTimeDataMapper;
...
public class RealTimeDataServiceImpl extends ServiceImpl<RealTimeDataMapper, RealTimeData> implements IRealTimeDataService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, RealTimeData realTimeData) {
        QueryWrapper<RealTimeData> queryWrapper = QueryGenerator.initQueryWrapper(realTimeData, paramMap);
        List<RealTimeData> list = list(queryWrapper);
        EasyExcel.write(outputStream, RealTimeData.class).sheet("实时数据信息表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, RealTimeData realTimeData) {
        QueryWrapper<RealTimeData> queryWrapper = QueryGenerator.initQueryWrapper(realTimeData, paramMap);
        List<RealTimeData> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<RealTimeData> result = listJson.stream().map(e -> JSON.toJavaObject(e,RealTimeData.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), RealTimeData.class, new ReadListener<RealTimeData>() {

                private List<RealTimeData> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(RealTimeData data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/RealTimeData.xlsx");
            // 方式一：路径
