...
import com.hanwei.videomanager.mapper.VideoInfoMapper;
import com.hanwei.videomanager.service.IVideoInfoService;
...
public class VideoInfoServiceImpl extends ServiceImpl<VideoInfoMapper, VideoInfo> implements IVideoInfoService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, VideoInfo videoInfo) {
        QueryWrapper<VideoInfo> queryWrapper = QueryGenerator.initQueryWrapper(videoInfo, paramMap);
        List<VideoInfo> list = list(queryWrapper);
        EasyExcel.write(outputStream, VideoInfo.class).sheet("视频信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, VideoInfo videoInfo) {
        QueryWrapper<VideoInfo> queryWrapper = QueryGenerator.initQueryWrapper(videoInfo, paramMap);
        List<VideoInfo> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<VideoInfo> result = listJson.stream().map(e -> JSON.toJavaObject(e,VideoInfo.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), VideoInfo.class, new ReadListener<VideoInfo>() {

                private List<VideoInfo> cachedDataList = new ArrayList<>();
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/VideoInfo.xlsx");
            // 方式一：路径
