package com.hanwei.scene.service.impl;
...
import com.hanwei.scene.mapper.SceneBaseMapper;
 ...
 */
@Service
public class SceneBaseServiceImpl extends ServiceImpl<SceneBaseMapper, SceneBase> implements ISceneBaseService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, SceneBase sceneBase) {
        QueryWrapper<SceneBase> queryWrapper = QueryGenerator.initQueryWrapper(sceneBase, paramMap);
        List<SceneBase> list = list(queryWrapper);
        EasyExcel.write(outputStream, SceneBase.class).sheet("场景基础信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, SceneBase sceneBase) {
        QueryWrapper<SceneBase> queryWrapper = QueryGenerator.initQueryWrapper(sceneBase, paramMap);
        List<SceneBase> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<SceneBase> result = listJson.stream().map(e -> JSON.toJavaObject(e,SceneBase.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), SceneBase.class, new ReadListener<SceneBase>() {

                private List<SceneBase> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(SceneBase data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/SceneBase.xlsx");
            // 方式一：路径
