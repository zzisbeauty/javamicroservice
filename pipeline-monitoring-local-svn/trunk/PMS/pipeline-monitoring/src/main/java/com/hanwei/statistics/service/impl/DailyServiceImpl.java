...
import com.hanwei.statistics.mapper.DailyMapper;
...
public class DailyServiceImpl extends ServiceImpl<DailyMapper, Daily> implements IDailyService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Daily daily) {
        QueryWrapper<Daily> queryWrapper = QueryGenerator.initQueryWrapper(daily, paramMap);
        List<Daily> list = list(queryWrapper);
        EasyExcel.write(outputStream, Daily.class).sheet("日汇聚信息表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Daily daily) {
        QueryWrapper<Daily> queryWrapper = QueryGenerator.initQueryWrapper(daily, paramMap);
        List<Daily> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Daily> result = listJson.stream().map(e -> JSON.toJavaObject(e,Daily.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Daily.class, new ReadListener<Daily>() {

                private List<Daily> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Daily data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Daily.xlsx");
            // 方式一：路径
