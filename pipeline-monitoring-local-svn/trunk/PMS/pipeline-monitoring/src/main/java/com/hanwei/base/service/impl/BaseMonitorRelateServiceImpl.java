...
import com.hanwei.base.mapper.BaseMonitorRelateMapper;
...
public class BaseMonitorRelateServiceImpl extends ServiceImpl<BaseMonitorRelateMapper, BaseMonitorRelate> implements IBaseMonitorRelateService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, BaseMonitorRelate baseMonitorRelate) {
        QueryWrapper<BaseMonitorRelate> queryWrapper = QueryGenerator.initQueryWrapper(baseMonitorRelate, paramMap);
        List<BaseMonitorRelate> list = list(queryWrapper);
        EasyExcel.write(outputStream, BaseMonitorRelate.class).sheet("监测点关联信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, BaseMonitorRelate baseMonitorRelate) {
        QueryWrapper<BaseMonitorRelate> queryWrapper = QueryGenerator.initQueryWrapper(baseMonitorRelate, paramMap);
        List<BaseMonitorRelate> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<BaseMonitorRelate> result = listJson.stream().map(e -> JSON.toJavaObject(e,BaseMonitorRelate.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), BaseMonitorRelate.class, new ReadListener<BaseMonitorRelate>() {

                private List<BaseMonitorRelate> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(BaseMonitorRelate data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/BaseMonitorRelate.xlsx");
            // 方式一：路径
