...
import com.hanwei.base.mapper.BaseCollectorMapper;
...
public class BaseCollectorServiceImpl extends ServiceImpl<BaseCollectorMapper, BaseCollector> implements IBaseCollectorService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, BaseCollector baseCollector) {
        QueryWrapper<BaseCollector> queryWrapper = QueryGenerator.initQueryWrapper(baseCollector, paramMap);
        List<BaseCollector> list = list(queryWrapper);
        EasyExcel.write(outputStream, BaseCollector.class).sheet("采集器").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, BaseCollector baseCollector) {
        QueryWrapper<BaseCollector> queryWrapper = QueryGenerator.initQueryWrapper(baseCollector, paramMap);
        List<BaseCollector> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<BaseCollector> result = listJson.stream().map(e -> JSON.toJavaObject(e,BaseCollector.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), BaseCollector.class, new ReadListener<BaseCollector>() {

                private List<BaseCollector> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(BaseCollector data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/BaseCollector.xlsx");
            // 方式一：路径
