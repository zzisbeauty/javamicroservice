...
import com.hanwei.statistics.mapper.MonthlyMapper;
...
public class MonthlyServiceImpl extends ServiceImpl<MonthlyMapper, Monthly> implements IMonthlyService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, Monthly monthly) {
        QueryWrapper<Monthly> queryWrapper = QueryGenerator.initQueryWrapper(monthly, paramMap);
        List<Monthly> list = list(queryWrapper);
        EasyExcel.write(outputStream, Monthly.class).sheet("月度汇聚信息表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, Monthly monthly) {
        QueryWrapper<Monthly> queryWrapper = QueryGenerator.initQueryWrapper(monthly, paramMap);
        List<Monthly> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<Monthly> result = listJson.stream().map(e -> JSON.toJavaObject(e,Monthly.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), Monthly.class, new ReadListener<Monthly>() {

                private List<Monthly> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(Monthly data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/Monthly.xlsx");
            // 方式一：路径
