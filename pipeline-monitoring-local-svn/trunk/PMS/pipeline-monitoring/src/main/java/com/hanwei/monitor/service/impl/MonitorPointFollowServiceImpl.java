...
import com.hanwei.monitor.mapper.MonitorPointFollowMapper;
 ...
 */
@Service
public class MonitorPointFollowServiceImpl extends ServiceImpl<MonitorPointFollowMapper, MonitorPointFollow> implements IMonitorPointFollowService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, MonitorPointFollow monitorPointFollow) {
        QueryWrapper<MonitorPointFollow> queryWrapper = QueryGenerator.initQueryWrapper(monitorPointFollow, paramMap);
        List<MonitorPointFollow> list = list(queryWrapper);
        EasyExcel.write(outputStream, MonitorPointFollow.class).sheet("监测点关注信息").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, MonitorPointFollow monitorPointFollow) {
        QueryWrapper<MonitorPointFollow> queryWrapper = QueryGenerator.initQueryWrapper(monitorPointFollow, paramMap);
        List<MonitorPointFollow> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<MonitorPointFollow> result = listJson.stream().map(e -> JSON.toJavaObject(e,MonitorPointFollow.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), MonitorPointFollow.class, new ReadListener<MonitorPointFollow>() {

                private List<MonitorPointFollow> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(MonitorPointFollow data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/MonitorPointFollow.xlsx");
            // 方式一：路径
