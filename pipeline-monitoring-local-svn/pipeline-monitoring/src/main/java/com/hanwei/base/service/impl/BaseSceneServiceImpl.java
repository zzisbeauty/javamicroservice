...
import com.hanwei.base.mapper.BaseSceneMapper;
...
public class BaseSceneServiceImpl extends ServiceImpl<BaseSceneMapper, BaseScene> implements IBaseSceneService {
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, BaseScene baseScene) {
        QueryWrapper<BaseScene> queryWrapper = QueryGenerator.initQueryWrapper(baseScene, paramMap);
        List<BaseScene> list = list(queryWrapper);
        EasyExcel.write(outputStream, BaseScene.class).sheet("场景表").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, BaseScene baseScene) {
        QueryWrapper<BaseScene> queryWrapper = QueryGenerator.initQueryWrapper(baseScene, paramMap);
        List<BaseScene> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<BaseScene> result = listJson.stream().map(e -> JSON.toJavaObject(e,BaseScene.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), BaseScene.class, new ReadListener<BaseScene>() {

                private List<BaseScene> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(BaseScene data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/BaseScene.xlsx");
            // 方式一：路径
