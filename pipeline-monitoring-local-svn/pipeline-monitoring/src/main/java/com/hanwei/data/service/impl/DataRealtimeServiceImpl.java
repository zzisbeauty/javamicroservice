package com.hanwei.data.service.impl;
...
import com.hanwei.data.mapper.DataRealtimeMapper;
...
public class DataRealtimeServiceImpl extends ServiceImpl<DataRealtimeMapper, DataRealtime> implements IDataRealtimeService {
   ...
   private final IBaseMonitorService baseMonitorService;
     ...
     */
    @Override
    public void exportData(ServletOutputStream outputStream, Map paramMap, DataRealtime dataRealtime) {
        QueryWrapper<DataRealtime> queryWrapper = QueryGenerator.initQueryWrapper(dataRealtime, paramMap);
        List<DataRealtime> list = list(queryWrapper);
        EasyExcel.write(outputStream, DataRealtime.class).sheet("实时数据").doWrite(list);
    }
     ...
     */
    @Override
    public String exportDataToBase64(Map paramMap, DataRealtime dataRealtime) {
        QueryWrapper<DataRealtime> queryWrapper = QueryGenerator.initQueryWrapper(dataRealtime, paramMap);
        List<DataRealtime> list = list(queryWrapper);
        //字典值转换
//        List<JSON> listJson = commonApi.translateResultByDict(list);
//        List<DataRealtime> result = listJson.stream().map(e -> JSON.toJavaObject(e,DataRealtime.class)).collect(Collectors.toList());
    ...
    public Result<?> importData(MultipartFile file) {
        try {
            EasyExcel.read(file.getInputStream(), DataRealtime.class, new ReadListener<DataRealtime>() {

                private List<DataRealtime> cachedDataList = new ArrayList<>();
                ...
                @SneakyThrows
                @Override
                public void invoke(DataRealtime data, AnalysisContext context) {
                    //---------------处理字典转义
...
//                    data.setRiskType(text);
    ...
    public String getImportTemplate() {
        ...
        try {
            // 模版文件
            ClassPathResource classPathResource = new ClassPathResource("template/DataRealtime.xlsx");
            // 方式一：路径
